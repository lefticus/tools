...
struct CountsMovesCopies
{
  ...
  constexpr CountsMovesCopies(CountsMovesCopies &&other) noexcept { ++other.move_constructed_from; }
  constexpr CountsMovesCopies(const CountsMovesCopies &other) noexcept { ++other.copy_constructed_from; }
...
template<typename T> constexpr auto can_construct_ref()
{
  return requires(T mct) { lefticus::tools::moving_ref<T>{ std::move(mct) }; };
}

template<typename T> constexpr auto can_construct_ref_const()
{
  return requires(const T mct) { lefticus::tools::moving_ref<T>{ std::move(mct) }; };
}

template<typename T> constexpr auto can_assign()
{
  return requires(T lhs, T mct) { lhs = lefticus::tools::moving_ref<T>{ std::move(mct) }; };
}


template<typename T> constexpr auto can_assign_const()
{
  return requires(T lhs, const T mct) { lhs = lefticus::tools::moving_ref<T>{ std::move(mct) }; };
}

template<typename T> constexpr auto can_construct()
{
  return requires(T mct) { T{ lefticus::tools::moving_ref<T>{ std::move(mct) } }; };
}


template<typename T> constexpr auto can_construct_const()
{
