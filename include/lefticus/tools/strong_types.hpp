...
template<typename LHS, typename RHS>
concept dividable = requires(LHS lhs, RHS rhs)
{
  divide(lhs, rhs);
};

template<typename LHS>
concept negatable = requires(LHS lhs)
{
  negate(lhs);
};

template<typename LHS, typename RHS>
concept orderable = requires(LHS lhs, RHS rhs)
{
  order(lhs, rhs);
};

template<typename LHS, typename RHS>
concept equatable = requires(LHS lhs, RHS rhs)
{
  equate(lhs, rhs);
};
...
template<typename Underlying, typename Tag, auto Validator = &null_validator<Underlying>> struct strong_alias
{
  ...
  constexpr explicit strong_alias(Param &&...param) requires std::is_constructible_v<Underlying, decltype(param)...>
  ...
  [[nodiscard]] auto operator<=>(const strong_alias &rhs) const noexcept
    requires(orderable<strong_alias, strong_alias>) = default;

  [[nodiscard]] bool operator==(const strong_alias &rhs) const noexcept
    requires(equatable<strong_alias, strong_alias>) = default;
...
private : Underlying data;
};

template<typename LHS, typename RHS>
[[nodiscard]] constexpr auto operator-(LHS &&lhs) noexcept(noexcept(-lhs.get()))
  -> decltype(negate(lhs)) requires(negatable<LHS>)
{
...
[[nodiscard]] constexpr auto operator+(LHS &&lhs, RHS &&rhs) noexcept(noexcept(lhs.get() + rhs.get()))
  -> decltype(add(lhs, rhs)) requires(addable<LHS, RHS>)
{
...
template<typename LHS, typename RHS>
[[nodiscard]] constexpr auto operator-(LHS &&lhs, RHS &&rhs) noexcept(noexcept(lhs.get() - rhs.get()))
  -> decltype(subtract(lhs, rhs)) requires(subtractable<LHS, RHS>)
{
...
template<typename LHS, typename RHS>
[[nodiscard]] constexpr auto operator*(LHS &&lhs, RHS &&rhs) noexcept(noexcept(lhs.get() * rhs.get()))
  -> decltype(multiply(lhs, rhs)) requires(multipliable<LHS, RHS>)
{
...
template<typename LHS, typename RHS>
[[nodiscard]] constexpr auto operator/(LHS &&lhs, RHS &&rhs) noexcept(noexcept(lhs.get() / rhs.get()))
  -> decltype(divide(lhs, rhs)) requires(dividable<LHS, RHS>)
{
  return decltype(divide(lhs, rhs)){ std::forward<LHS>(lhs).get() / std::forward<RHS>(rhs).get() };
}

template<typename To, typename From>
[[nodiscard]] constexpr auto strong_cast(From &&from) -> To requires(casts_to<From, To>)
{
